...
contract FaultDisputeGame is IFaultDisputeGame, Clone, ISemver {
    ...
    function resolve() external returns (GameStatus status_) {
        ...
        // Unlock withdrawals, again. Doesn't hurt to do this one last time at the root level.
        WETH.unlock();
    ...
    function resolveClaim(uint256 _claimIndex) external payable {
        ...
        if (challengeIndicesLen == 0 && _claimIndex != 0) {
            ...
            // See comment about same function call below to understand this call.
            WETH.unlock();
        ...
        // Unlock withdrawals. Every time this function is called, a timer is started that must
        // elapse before WETH can be withdrawn back into this contract. Triggering this function
        // here is fine because the timer is only important when dealing with claims that resolve
        // incorrectly. Someone cannot, for instance, trigger the function once, wait for the timer
        // to expire, and then resolve another claim incorrectly and withdraw the WETH. Once the
        // second claim is resolved, the timer is reset and the user must wait. No real need to
        // worry about people making lots of claims to try to delay withdrawals as there's already
        // a disincentive to do so in the form of the bond.
        WETH.unlock();
    }
